class Api::V1::UsersController < ApiController
  ...
  def show
    @user = User.includes({ pictures: [:comments, :bookmarks, :likes, :liked_users, :theme] }, :bookmarks, :comments, :likes, :liked_pictures ,:followings, :followers, :bookmarks, :bookmark_pictures).find(params[:id])
  ...
  def best_users
    picture_like_count = {}
    User.all.includes({ pictures: [:comments, :likes, :liked_users, :bookmarks, :theme] }, :liked_pictures, :bookmarks, :bookmark_pictures, :comments, :likes, :followings, :followers).without_guests.each do |user|
